...
export default {
  ...
  methods: {
            ...
            if (data.code == 0) {
              ...
              for (
                ...
                if (
                  ...
                  this.provinceCode = this.searchCountryAreaCodeListPage[
                    i
                  ].code;
    ...
    mydeler() {
      ...
      this.$http.post(Wit.Dealer, param).then(res => {
        ...
        if (data.code == 0) {
          ...
          if (data.data.total <= this.size) {
            //如果总条数小于等于请求的数据条数,不在请求加载更多
            this.loadEnd = true;
          }
        } else {
          Toast({
    ...
    getIosLocation(locationMes) {
      //IOS调用,H5获取ios定位信息
      console.log("jinru");
      console.log(locationMes + "dealer");
