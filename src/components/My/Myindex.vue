  ...
  <div>
    <div class="mytop" :style="$statusBarHeightObj">
      ...
      <div class="mytophead flex between cocenter">
        <div
          style="padding-left: 0.3rem;padding-right: 0.2rem;position:relative"
          class="flex cocenter"
        >
          <div
            @click="edict()"
            style="width: 1.2rem;height: 1.2rem;border-radius:50%;overflow:hidden"
          >
            <img v-if="Personal.headUrl" :src="Personal.headUrl" alt style="width:100%;height:100%">
            <img
              v-else
              src="../../../static/images/discover/normalhead.png"
              alt
          ...
          <div class="flex column" style="margin-left: 0.2rem;">
            <div class="flex cocenter" style="overflow: hidden">
              <div v-if="this.$store.state.userId">
                <span
                  v-if="Personal.userName"
                  style="color: #fff;font-size: 0.32rem;font-weight: bold"
                >{{Personal.userName}}</span>
              ...
              <img
                v-if="Personal.sex==1"
                src="../../../static/images/my/gender_man@2x.png"
                alt
                style="width: 0.28rem;height: 0.28rem"
              >
              <img
                v-if="Personal.sex==2"
                src="../../../static/images/my/gender_woman.png"
                alt
                style="width: 0.28rem;height: 0.28rem"
              >
            </div>
            <span
              @click="ToScore()"
              style="margin-top: 0.16rem;font-size: 0.22rem;color: #dcf5ff;"
            >{{integral}}&nbsp;积分&nbsp;></span>
          </div>
        </div>
    ...
    <mt-popup v-model="popupVisible" position="middle">
      <div style="display: flex;justify-content: center;align-items: center;  margin-top:.3rem">
        <img
          src="../../../static/images/my/complete@3x.png"
          alt
          style="width:1.2rem;display:block;"
        >
      </div>
      <span
        style="font-size:.32rem;color:#222;margin-top:.4rem;text-align:center;display:block"
      >签到成功</span>
      <span style="font-size:.24rem;color:#444;margin-top:.3rem;text-align:center;display:block">
        获得
        <span style="color:#49BBFF">{{this.num}}</span> 积分可兑换会员商品
      </span>
    </mt-popup>
  </div>
</template>
...
.loginto {
  font: 500 0.6rem/0.4rem "PingFang-SC-Medium";
  color: #fff;
}
</style>
