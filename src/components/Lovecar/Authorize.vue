...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.returnSuccess) {
            Toast({
              message:
                "友情提示：请告知被授权用户重新登入APP，即可体验远程车控功能",
              position: "middle",
              duration: 3000
            });
            setTimeout(() => {
    ...
    next() {
      ...
      this.thirdStartParty =
        this.start.replace(/\-/g, "/").split(" ")[1] * 60 * 60 * 1000;
      this.xia = this.end.replace(/\-/g, "/").split(" ")[0];
      this.thirdEndParty =
        this.end.replace(/\-/g, "/").split(" ")[1] * 60 * 60 * 1000;
      ...
      if (this.shang > this.xia) {
        ...
        this.$http
          .post(Lovecar.Longrange, param, this.$store.state.tsppin)
          .then(res => {
            if (res.data.returnSuccess) {
              this.MyBus();
            } else {
